...
type Client interface {
	...
	PersistCrawlVisit(ctx context.Context, crawlID int, peerID peer.ID, enodeStr string, ENR *enode.Node, maddrs []ma.Multiaddr, protocols []string, agentVersion string, connectDuration time.Duration, crawlDuration time.Duration, visitStartedAt time.Time, visitEndedAt time.Time, connectErrorStr string, crawlErrorStr string, properties null.JSON) (*InsertVisitResult, error)
