...
func (c *Crawler) Work(ctx context.Context, task PeerInfo) (core.CrawlResult[PeerInfo], error) {
	...
	cr := core.CrawlResult[PeerInfo]{
		...
		ENR:                 discv4Result.ENR,
		EnodeStr:            discv4Result.EnodeStr,
	}
...
func (c *Crawler) crawlDiscV4(ctx context.Context, pi PeerInfo) <-chan DiscV4Result {
	...
	go func() {
		...
		result.EnodeStr = pi.Node.String()
		if err != nil {
			result.ENR = nil
			log.Println("no ENR found for", pi.Node.String())
		} else {
			result.ENR = enr
			now := time.Now()
			result.RespondedAt = &now
			log.Println("ENR found for", pi.Node.String(), enr.String())
		}
